	...
	<main>
		...
		<form action="board04">
			...
			<table border=1 width="50%">
				...
				<c:forEach var="list04dto" items="${list04}">
					<tr>
						<td>${list04dto.b_no04}</td>
						<td align="center"><a href="/adminpage/content4/${list04dto.b_no04}">${list04dto.title}</a></td>
						<td align="center">${list04dto.id}</td>
						<td align="center"><fmt:formatDate value="${list04dto.date}" dateStyle="short"/></td>
						<td align="center"><input type="checkbox" name="no" value="${list04dto.b_no04}"/></td>
					</tr>
				</c:forEach>
			</table>
			<div id="page">
				<c:if test="${begin > pageNum }">
					<a href="board04?p=${begin-1}&search=${search}&searchn=${searchn}">[이전]</a>
				</c:if>
				<c:forEach begin="${begin }" end="${end}" var="i">
					<a href="board04?p=${i}&search=${search}&searchn=${searchn}">${i}</a>
				</c:forEach>
				<c:if test="${end < totalPages }">
					<a href="board04?p=${end+1}&search=${search}&searchn=${searchn}">[다음]</a>
				</c:if>
			</div>
