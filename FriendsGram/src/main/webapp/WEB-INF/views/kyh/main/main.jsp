<%@ page contentType="text/html; charset=UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css?family=Inter&display=swap"
	rel="stylesheet" />
<link
	href="https://fonts.googleapis.com/css?family=Epilogue&display=swap"
	rel="stylesheet" />
<link href="https://fonts.googleapis.com/css?family=Lexend&display=swap"
	rel="stylesheet" />
<link href="./css/main.css" rel="stylesheet" />
<style>
* {
  box-sizing: border-box;
}
body {
  font-size: 14px;
}
.v147_2186 {
  width: 100%;
  height: 1333px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: relative;
  top: 0px;
  left: 0px;
  box-shadow: 0px 3px 6px rgba(0.07058823853731155, 0.05882352963089943, 0.1568627506494522, 0.11999999731779099);
  overflow: hidden;
}
.v147_2187 {
  width: 100%;
  height: 587px;
  background: rgba(12,146,0,1);
  opacity: 1;
  position: absolute;
  top: 73px;
  left: 0px;
  overflow: hidden;
}
.v147_2188 {
  width: 357px;
  height: 259px;
  background: url("/images/v147_2188.png");
  background-repeat: no-repeat;
  background-position: center center;
  background-size: cover;
  opacity: 1;
  position: absolute;
  top: 5687px;
  left: 1044px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2189 {
  width: 189px;
  height: 52px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 333px;
  left: 1318px;
  border-top-left-radius: 26px;
  border-top-right-radius: 26px;
  border-bottom-left-radius: 26px;
  border-bottom-right-radius: 26px;
  overflow: hidden;
}
.v147_2190 {
  width: 169px;
  color: rgba(50,56,66,1);
  position: absolute;
  top: 12px;
  left: 20px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 18px;
  opacity: 1;
  text-align: left;
}
.v147_2191 {
  width: 270px;
  color: rgba(255,255,255,1);
  position: absolute;
  top: 113px;
  left: 1318px;
  font-family: Epilogue;
  font-weight: Regular;
  font-size: 48px;
  opacity: 1;
  text-align: left;
}
.v147_2192 {
  width: 878px;
  height: 587px;
  background: url("/images/v147_2192.png");
  background-repeat: no-repeat;
  background-position: center center;
  background-size: cover;
  opacity: 1;
  position: relative;
  top: 0px;
  left: 0px;
  border-top-right-radius: 64px;
  border-bottom-right-radius: 64px;
  overflow: hidden;
}
.v147_2193 {
  width: 100%;
  height: 72px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 1px;
  left: 0px;
  overflow: hidden;
}
.v147_2194 {
  width: 110px;
  height: 40px;
  background: rgba(12,146,0,1);
  opacity: 1;
  position: absolute;
  top: 16px;
  left: 1657px;
  border-top-left-radius: 20px;
  border-top-right-radius: 20px;
  border-bottom-left-radius: 20px;
  border-bottom-right-radius: 20px;
  overflow: hidden;
}
.v147_2195 {
  width: 82px;
  color: rgba(255,255,255,1);
  position: absolute;
  top: 6px;
  left: 28px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 18px;
  opacity: 1;
  text-align: left;
}
.v147_2196 {
  width: 100%;
  height: 73px;
  background: rgba(0,0,0,0);
  opacity: 1;
  position: absolute;
  top: 1px;
  left: 379px;
  overflow: hidden;
}
.v147_2197 {
  width: 153px;
  height: 73px;
  background: rgba(0,0,0,0);
  opacity: 1;
  position: relative;
  top: 0px;
  left: 0px;
  overflow: hidden;
}
.v147_2198 {
  width: 106px;
  color: rgba(86,94,108,1);
  position: absolute;
  top: 22px;
  left: 29px;
  font-family: Inter;
  font-weight: Bold;
  font-size: 18px;
  opacity: 1;
  text-align: left;
}
.v147_2199 {
  width: 172px;
  height: 73px;
  background: rgba(0,0,0,0);
  opacity: 1;
  position: absolute;
  top: 0px;
  left: 154px;
  overflow: hidden;
}
.v147_2200 {
  width: 132px;
  color: rgba(0,0,0,1);
  position: absolute;
  top: 22px;
  left: 30px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 18px;
  opacity: 1;
  text-align: left;
}
.v147_2201 {
  width: 118px;
  height: 73px;
  background: rgba(0,0,0,0);
  opacity: 1;
  position: absolute;
  top: 0px;
  left: 326px;
  overflow: hidden;
}
.v147_2202 {
  width: 89px;
  color: rgba(86,94,108,1);
  position: absolute;
  top: 22px;
  left: 30px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 18px;
  opacity: 1;
  text-align: left;
}
.v147_2203 {
  width: 172px;
  height: 73px;
  background: rgba(0,0,0,0);
  opacity: 1;
  position: absolute;
  top: 0px;
  left: 445px;
  overflow: hidden;
}
.v147_2204 {
  width: 144px;
  color: rgba(86,94,108,1);
  position: absolute;
  top: 22px;
  left: 29px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 18px;
  opacity: 1;
  text-align: left;
}
.v147_2205 {
  width: 136px;
  height: 73px;
  background: rgba(0,0,0,0);
  opacity: 1;
  position: absolute;
  top: 0px;
  left: 617px;
  overflow: hidden;
}
.v147_2206 {
  width: 107px;
  color: rgba(86,94,108,1);
  position: absolute;
  top: 22px;
  left: 30px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 18px;
  opacity: 1;
  text-align: left;
}
.v147_2207 {
  width: 36px;
  height: 36px;
  background: rgba(12,146,0,1);
  opacity: 1;
  position: absolute;
  top: 19px;
  left: 1853px;
  border-top-left-radius: 18px;
  border-top-right-radius: 18px;
  border-bottom-left-radius: 18px;
  border-bottom-right-radius: 18px;
  overflow: hidden;
}
.v147_2208 {
  width: 18px;
  height: 18px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 9px;
  left: 9px;
  overflow: hidden;
}
.v147_2209 {
  width: 10px;
  height: 4px;
  background: url("/images/v147_2209.png");
  opacity: 1;
  position: absolute;
  top: 10px;
  left: 4px;
  border: 1.0800000429153442px solid rgba(255,255,255,1);
}
.v147_2210 {
  width: 6px;
  height: 6px;
  background: url("/images/v147_2210.png");
  opacity: 1;
  position: absolute;
  top: 2px;
  left: 6px;
  border: 1.0800000429153442px solid rgba(255,255,255,1);
}
.v147_2211 {
  width: 44px;
  height: 44px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 16px;
  left: 1786px;
  overflow: hidden;
}
.v147_2212 {
  width: 30px;
  height: 12px;
  background: url("/images/v147_2212.png");
  opacity: 1;
  position: absolute;
  top: 9px;
  left: 7px;
  border: 2.640000104904175px solid rgba(23,26,31,1);
}
.v147_2213 {
  width: 33px;
  height: 26px;
  background: url("/images/v147_2213.png");
  opacity: 1;
  position: absolute;
  top: 8px;
  left: 6px;
  border: 2.640000104904175px solid rgba(23,26,31,1);
}
.v147_2214 {
  width: 201px;
  height: 72px;
  background: url("/images/v147_2214.png");
  background-repeat: no-repeat;
  background-position: center center;
  background-size: cover;
  opacity: 1;
  position: absolute;
  top: 0px;
  left: 92px;
  overflow: hidden;
}
.v147_2215 {
  width: 100%;
  height: 493px;
  background: rgba(255,255,255,0.5);
  opacity: 1;
  position: absolute;
  top: 660px;
  left: 0px;
  overflow: hidden;
}
.v147_2216 {
  width: 20px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 1014px;
  left: 615px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: right;
}
.v147_2217 {
  width: 521px;
  background: url("/images/v147_2217.png");
  opacity: 1;
  position: absolute;
  top: 69px;
  left: 91px;
  border: 1px solid rgba(0,0,0,1);
}
.v147_2218 {
  width: 305px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 13px;
  left: 91px;
  font-family: Lexend;
  font-weight: Bold;
  font-size: 40px;
  opacity: 1;
  text-align: left;
}
.v147_2219 {
  width: 521px;
  height: 327px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 83px;
  left: 91px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  box-shadow: 0px 0px 1px rgba(0.09019608050584793, 0.10196078568696976, 0.12156862765550613, 0.07000000029802322);
  overflow: hidden;
}
.v147_2220 {
  width: 100%;
  height: 173px;
  background: rgba(248,249,250,1);
  opacity: 1;
  position: absolute;
  top: 85px;
  left: 733px;
  border-top-left-radius: 48px;
  border-top-right-radius: 48px;
  border-bottom-left-radius: 48px;
  border-bottom-right-radius: 48px;
  overflow: hidden;
}
.v147_2221 {
  width: 521px;
  background: url("/images/v147_2221.png");
  opacity: 1;
  position: absolute;
  top: 69px;
  left: 724px;
  border: 1px solid rgba(0,0,0,1);
}
.v147_2222 {
  width: 462px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 13px;
  left: 724px;
  font-family: Lexend;
  font-weight: Bold;
  font-size: 40px;
  opacity: 1;
  text-align: left;
}
.v147_2223 {
  width: 949px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 100px;
  left: 755px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2224 {
  width: 949px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 3px;
  left: 0px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 20px;
  opacity: 1;
  text-align: left;
}
.v147_2225 {
  width: 267px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 149px;
  left: 755px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2226 {
  width: 255px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2227 {
  width: 267px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 149px;
  left: 1097px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2228 {
  width: 255px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2229 {
  width: 267px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 149px;
  left: 1438px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2230 {
  width: 255px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2231 {
  width: 132px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 201px;
  left: 755px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2232 {
  width: 120px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2233 {
  width: 115px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 201px;
  left: 1036px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2234 {
  width: 103px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2235 {
  width: 100%;
  height: 173px;
  background: rgba(248,249,250,1);
  opacity: 1;
  position: absolute;
  top: 289px;
  left: 733px;
  border-top-left-radius: 48px;
  border-top-right-radius: 48px;
  border-bottom-left-radius: 48px;
  border-bottom-right-radius: 48px;
  overflow: hidden;
}
.v147_2236 {
  width: 949px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 304px;
  left: 755px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2237 {
  width: 949px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 3px;
  left: 0px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 20px;
  opacity: 1;
  text-align: left;
}
.v147_2238 {
  width: 267px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 353px;
  left: 755px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2239 {
  width: 247px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2240 {
  width: 267px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 353px;
  left: 1097px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2241 {
  width: 255px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2242 {
  width: 267px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 353px;
  left: 1438px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2243 {
  width: 255px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2244 {
  width: 132px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 405px;
  left: 755px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2245 {
  width: 120px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2246 {
  width: 115px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 405px;
  left: 1036px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2247 {
  width: 103px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2248 {
  width: 115px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 201px;
  left: 904px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2249 {
  width: 103px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2250 {
  width: 115px;
  height: 35px;
  background: rgba(255,255,255,1);
  opacity: 1;
  position: absolute;
  top: 405px;
  left: 904px;
  border-top-left-radius: 4px;
  border-top-right-radius: 4px;
  border-bottom-left-radius: 4px;
  border-bottom-right-radius: 4px;
  overflow: hidden;
}
.v147_2251 {
  width: 103px;
  color: rgba(23,26,31,1);
  position: absolute;
  top: 7px;
  left: 12px;
  font-family: Inter;
  font-weight: Regular;
  font-size: 14px;
  opacity: 1;
  text-align: left;
}
.v147_2252 {
  width: 100%;
  height: 183px;
  background: rgba(30,33,40,0.6800000071525574);
  opacity: 1;
  position: absolute;
  top: 1152px;
  left: 0px;
  border-top-left-radius: 48px;
  border-top-right-radius: 48px;
  overflow: hidden;
}
.v147_2253 {
  width: 130px;
  color: rgba(255,255,255,1);
  position: absolute;
  top: 49px;
  left: 901px;
  font-family: Epilogue;
  font-weight: Bold;
  font-size: 24px;
  opacity: 1;
  text-align: left;
}
.v147_2254 {
  width: 268px;
  color: rgba(255,255,255,1);
  position: absolute;
  top: 49px;
  left: 1365px;
  font-family: Epilogue;
  font-weight: Bold;
  font-size: 24px;
  opacity: 1;
  text-align: left;
}
.v147_2255 {
  width: 339px;
  color: rgba(255,255,255,1);
  position: absolute;
  top: 103px;
  left: 716px;
  font-family: Epilogue;
  font-weight: Regular;
  font-size: 16px;
  opacity: 1;
  text-align: left;
}
.v147_2256 {
  width: 406px;
  color: rgba(255,255,255,1);
  position: absolute;
  top: 103px;
  left: 1136px;
  font-family: Epilogue;
  font-weight: Regular;
  font-size: 16px;
  opacity: 1;
  text-align: left;
}
.v147_2257 {
  width: 268px;
  color: rgba(255,255,255,1);
  position: absolute;
  top: 49px;
  left: 558px;
  font-family: Epilogue;
  font-weight: Bold;
  font-size: 24px;
  opacity: 1;
  text-align: left;
}
.v147_2258 {
  width: 138px;
  color: rgba(255,255,255,1);
  position: absolute;
  top: 49px;
  left: 1136px;
  font-family: Epilogue;
  font-weight: Bold;
  font-size: 24px;
  opacity: 1;
  text-align: left;
}
.v147_2259 {
  width: 301px;
  height: 106px;
  background: url("/images/v147_2259.png");
  background-repeat: no-repeat;
  background-position: center center;
  background-size: cover;
  opacity: 1;
  position: absolute;
  top: 41px;
  left: 152px;
  overflow: hidden;
}
</style>
<title>Document</title>
</head>
<body>
	<div class="v147_2186">
		<div class="v147_2187">
			<div class="v147_2188"></div>
			<div class="v147_2189">
				<span class="v147_2190">프리랜서 찾아보기</span>
			</div>
			<span class="v147_2191">원하는 프리랜서를 찾아보세요!</span>
			<div class="v147_2192"></div>
		</div>
		<div class="v147_2193">
			<div class="v147_2194">
				<span class="v147_2195">로그인</span>
			</div>
			<div class="v147_2196">
				<div class="v147_2197">
					<sapn class="v147_2198">코딩 게시판</span>
				</div>
				<div class="v147_2199">
					<span class="v147_2200">프리랜서 검색</span>
				</div>
				<div class="v147_2201">
					<span class="v147_2202">팀 모집</span>
				</div>
				<div class="v147_2203">
					<span class="v147_2204">프리랜서 공고</span>
				</div>
				<div class="v147_2205">
					<span class="v147_2206">취업 공고</span>
				</div>
			</div>
			<div class="v147_2207">
				<div class="v147_2208">
					<div class="v147_2209"></div>
					<div class="v147_2210"></div>
				</div>
			</div>
			<div class="v147_2211">
				<div class="v147_2212"></div>
				<div class="v147_2213"></div>
			</div>
			<div class="v147_2214"></div>
		</div>
		<div class="v147_2215">
			<span class="v147_2216">↗</span>
			<div class="v147_2217"></div>
			<span class="v147_2218">공지사항</span>
			<div class="v147_2219"></div>
			<div class="v147_2220"></div>
			<div class="v147_2221"></div>
			<span class="v147_2222">프리랜서 공고</span>
			<div class="v147_2223">
				<span class="v147_2224">프리랜서 모집공고 제목</span>
			</div>
			<div class="v147_2225">
				<span class="v147_2226">예상 금액</span>
			</div>
			<div class="v147_2227">
				<span class="v147_2228">시작 예정일</span>
			</div>
			<div class="v147_2229">
				<span class="v147_2230">예상기간</span>
			</div>
			<div class="v147_2231">
				<span class="v147_2232">상주, 원격</span>
			</div>
			<div class="v147_2233">
				<span class="v147_2234">모집중</span>
			</div>
			<div class="v147_2235"></div>
			<div class="v147_2236">
				<span class="v147_2237">프리랜서 모집공고 제목</span>
			</div>
			<div class="v147_2238">
				<span class="v147_2239">예상 금액</span>
			</div>
			<div class="v147_2240">
				<span class="v147_2241">시작 예정일</span>
			</div>
			<div class="v147_2242">
				<span class="v147_2243">예상기간</span>
			</div>
			<div class="v147_2244">
				<span class="v147_2245">상주, 원격</span>
			</div>
			<div class="v147_2246">
				<span class="v147_2247">모집중</span>
			</div>
			<div class="v147_2248">
				<span class="v147_2249">사용언어</span>
			</div>
			<div class="v147_2250">
				<span class="v147_2251">사용언어</span>
			</div>
		</div>
		<div class="v147_2252">
			<span class="v147_2253">고객센터</span><span class="v147_2254">개인정보
				처리방침</span><span class="v147_2255">(주)프렌즈그램(대표이사 : 전재민)</span><span
				class="v147_2256">서울 특별시 종로구 종로 12길 15 코아빌딩 </span><span
				class="v147_2257">프리랜서 이용약관</span><span class="v147_2258">광고
				문의</span>
			<div class="v147_2259"></div>
		</div>
	</div>
</body>
</html>
</body>
</html>